...
function check_update()
    msg(u8'Проверка наличия обновлений...')
    ...
    if response and response ~= currentVersion then
        msg(u8"Найдено новое обновление! Вывожу окно для загрузки...")
        found_update[0] = not found_update[0]
    else
        mag(u8"У вас актуальная версия скрипта.")
    end
end
...
function updateScript(scriptUrl, scriptPath)
    ...
    if response and response ~= currentVersion then
        ...
        if success then
            msg(u8"Скрипт успешно обновлен. Перезагрузка..")
            thisScript():reload()
        else
            msg(u8"Неизвестная ошибка, не удалось обновить скрипт.")
        end
    else
        msg(u8"Скрипт уже является последней версией.")
    end
end
...
imgui.OnFrame(function() return found_update[0] end, function()
    ...
    if imgui.Button(fa.FORWARD..u8' ПРОПУСТИТЬ', imgui.ImVec2(650, 40)) then
       found_update[0] = not found_update[0]
       msg(u8'Обновление скрипта пропущено') 
	    end
		imgui.End()
		end)
...
if not isSampfuncsLoaded() or not isSampLoaded() then return end
    ...
    sampAddChatMessage("Активация: /db", -1)
    sampAddChatMessage('Автор Jake', -1)
    check_update()
    end 
