...
imgui.OnFrame(function() return OilMenu[0] end, function(player)
    ...
    if imgui.BeginTabBar('Tabs') then	           										  
        if imgui.BeginTabItem(fa.GEAR .. u8' Настройки инфобара') then
          ...
          ini.knopa.skipdialogi = skipdialogi[0]      msg('Теперь у вас' .. (skipdialogi[0] and ' автоматически будут скипаться лишние диалоги.' or ' не будут автоматически скипаться лишние диалоги.'))        
                    cfg_save() 
                end                      
                imgui.Separator()
...
function cfg_save()
inicfg.save(ini, "Truck HelperConfig")
end
