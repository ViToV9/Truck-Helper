...
function check_update()
    msg(u8'Ïðîâåðêà íàëè÷èÿ îáíîâëåíèé...')
    ...
    if response and response ~= currentVersion then
        msg(u8"Íàéäåíî íîâîå îáíîâëåíèå! Âûâîæó îêíî äëÿ çàãðóçêè...")
        found_update[0] = not found_update[0]
    else
        mag(u8"Ó âàñ àêòóàëüíàÿ âåðñèÿ ñêðèïòà.")
    end
end
...
function updateScript(scriptUrl, scriptPath)
    ...
    if response and response ~= currentVersion then
        ...
        if success then
            msg(u8"Ñêðèïò óñïåøíî îáíîâëåí. Ïåðåçàãðóçêà..")
            thisScript():reload()
        else
            msg(u8"Íåèçâåñòíàÿ îøèáêà, íå óäàëîñü îáíîâèòü ñêðèïò.")
        end
    else
        msg(u8"Ñêðèïò óæå ÿâëÿåòñÿ ïîñëåäíåé âåðñèåé.")
    end
end
...
imgui.OnFrame(function() return found_update[0] end, function()
    ...
    imgui.CenterText('Truck Helper ')
    imgui.PopFont()
    imgui.PushFont(updfont[20])
    imgui.PopFont()
    imgui.CenterText('')
    imgui.PushFont(updfont[33])
    imgui.CenterText(u8'Íàéäåíî íîâîå îáíîâëåíèå!')
    imgui.PopFont()
    imgui.CenterText('')
    imgui.CenterText('')
    imgui.PushFont(updfont[25])
    imgui.CenterText(u8'Áûëî íàéäåíî íîâîå îáíîâëåíèå ñêðèïòà.')
    imgui.CenterText(u8'Äëÿ ïðîäîëæåíèÿ ðàáîòû Âàì ')
    imgui.CenterText(u8'íåîáõîäèìî âûáðàòü îäíó èç äâóõ êíîïîê íèæå')
    imgui.PopFont()
    imgui.CenterText('')
    imgui.CenterText('')
    ...
    if imgui.Button(fa.FORWARD..u8' ÏÐÎÏÓÑÒÈÒÜ', imgui.ImVec2(650, 40)) then
       found_update[0] = not found_update[0]
       msg(u8'Îáíîâëåíèå ñêðèïòà ïðîïóùåíî') 
	    end
		imgui.End()
		end)
...
if not isSampfuncsLoaded() or not isSampLoaded() then return end
    ...
    sampAddChatMessage('Ñêðèïò Çàãðóæåí!', -1)
    sampAddChatMessage("Àêòèâàöèÿ: /db", -1)
    sampAddChatMessage('Àâòîð Jake', -1)
    check_update()
    end 
