...
script_author("@TelefonRedmi12c // Maksim(Jake)")
script_version("2.2")
----------------- [Библиотеки] ---------------------------
...
imgui.OnFrame(function() return OilMenu[0] end, function(player)
    ...
    if imgui.BeginTabBar('Tabs') then			  
        ...
        imgui.BeginChild('##1', imgui.ImVec2(589 * MONET_DPI_SCALE, 270 * MONET_DPI_SCALE), true)		
...
imgui.BeginChild('##1', imgui.ImVec2(589 * MONET_DPI_SCALE, 117 * MONET_DPI_SCALE), true)
    ...
    imgui.Text(fa.CIRCLE_USER..u8" Разработчик данного хелпера: TelefonRedmi12c // Maksim(Jake)")
				imgui.Separator()
...
script_reload()
	...
	imgui.SameLine()
	if imgui.Button(fa.ROTATE_RIGHT .. u8" Выключить ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * MONET_DPI_SCALE)) then
reload_script = true
script_unload() 
end
